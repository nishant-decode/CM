...
int main() {
    ...
    for (int i = 0; i < 5; i++) {
try{
            RootFinder R(fInterval[i], f[i],VARIABLE_LINEAR); // You need to change only the LINEAR
...
<<e.what() << endl;
    ...
    for (int i = 0; i < 5; i++) {
try{
            RootFinder R(pInterval[i], P[i],VARIABLE_LINEAR); // You need to change only the LINEAR
                                                        // for other root finding functions
            long COUNT = R.findRoot();
            cout <<setw(5) << i + 1
<<setw(10) << COUNT
